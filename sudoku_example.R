function (z = NULL, hist.len = 100, solve = TRUE, display = c("guess","windows", "tk"), hscale = 1.25, vscale = 1.25, ...) 
{
  ...
  if (solve) {
    cat("Solving...")
    zz <- solveSudoku(z, print.it = FALSE)
    cat("done!\n")
  }
  ...
  replot <- function() {
    ...
    plot(0.5:9.5, 0.5:9.5, type = "n", axes = FALSE, xlab = "", 
         ylab = "")
    cusr <<- par("usr")
    cplt <<- par("plt")
    segments(0.5:9.5, rep(0.5, 10), 0.5:9.5, rep(9.5, 10), 
             col = "grey")
    segments(rep(0.5, 10), 0.5:9.5, rep(9.5, 10), 0.5:9.5, 
             col = "grey")
    segments(c(0, 3, 6, 9) + 0.5, rep(0.5, 4), c(0, 3, 6, 
                                                 9) + 0.5, rep(9.5, 4), lwd = 3)
    segments(rep(0.5, 4), c(0, 3, 6, 9) + 0.5, rep(9.5, 4), 
             c(0, 3, 6, 9) + 0.5, lwd = 3)
  ...
  help.txt <- paste(" ?     -- this help", "1-9   -- insert digit", 
                    "0,' ' -- clear cell", "r     -- replot the puzzle", 
                    "q     -- quit", "h     -- hint/help", "c     -- correct wrong entries (show in red)", 
                    "u     -- undo last entry", "s     -- show number in cell", 
