...
evaluate(untrained_agent,make_random_agent())
...
trained_agent.30000 = nnet1.fit(trained_agent.21000$agent,ALPHA=0.1,LAMBDA=0.8,9000,FALSE,eval_freq)
...
```{r}
trained_agent = trained_agent.1000
cost_hist = trained_agent$cost
wv_hist = trained_agent$wv_hist
lv_hist = trained_agent$lv_hist

wv_hist = c(trained_agent.1000$wv_hist, trained_agent.2000$wv_hist)
lv_hist = c(trained_agent.1000$lv_hist, trained_agent.2000$lv_hist)

wv_hist = c(trained_agent.1000$wv_hist, trained_agent.2000$wv_hist, trained_agent.3000$wv_hist, trained_agent.4000$wv_hist)
lv_hist = c(trained_agent.1000$lv_hist, trained_agent.2000$lv_hist, trained_agent.3000$lv_hist, trained_agent.4000$lv_hist)

plot(0,0,xlim=c(0,length(cost_hist)),ylim=c(0,0.2), type="n", ylab="", xlab="Epochs")
...
points(x=seq(1,length(wv_hist)), y=unlist(wv_hist), col="red")
points(x=seq(1,length(lv_hist)), y=unlist(lv_hist), col="blue")
...
```{r}
eval = evaluate(trained_agent.1000$agent,make_random_agent(),300)
eval$p1
eval = evaluate(trained_agent.2000$agent,make_random_agent(),300)
eval$p1
eval = evaluate(trained_agent.3000$agent,make_random_agent(),300)
eval$p1
eval = evaluate(trained_agent.4000$agent,make_random_agent(),300)
eval$p1
eval = evaluate(trained_agent.5000$agent,make_random_agent(),300)
eval$p1
eval = evaluate(trained_agent.6000$agent,make_random_agent(),300)
eval$p1
eval = evaluate(trained_agent.7000$agent,make_random_agent(),300)
eval$p1
eval$avg_turns
```

```{r}
trained_agent = trained_agent.2000
winners = trained_agent$winner
p1_hist = trained_agent$games_p1
p2_hist = trained_agent$games_p2
i =985
p1 = p1_hist[[i]]
p2 = p2_hist[[i]]
winner = winners[[i]]
